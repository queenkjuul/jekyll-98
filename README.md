...
Theme should provide sane defaults, but there's lots to customize. For full configuration options, refer to [_config.yml](_config.yml). Add whatever settings you want *to your site's _config.yml*.
...
change how timestamps are shown

#### `social_links`

All available sites/icons are listed in the theme [`_config.yml`](_config.yml). Define a username, and the site will appear in the taskbar, as well as the Contact page (if enabled)
...
See [`_config.yml`](_config.yml) for details on setting up additional info beyond name + email.
